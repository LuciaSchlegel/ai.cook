...
export const RecipeFilterService = {
  ...
  filterRecipes({
    ...
    if (filter === "With Available Ingredients") {
      if (mode === "ai_flexible") {
        ...
        filtered = filtered.filter((recipe) =>
          this._hasAlmostAllIngredients(recipe, userIngredients, 2)
        );
        if (this.DEBUG_MODE)
          console.log(
            `✅ After AI Flexible Available filter: ${filtered.length} recipes`
          );
      } else {
        filtered = filtered.filter((recipe) =>
          this._hasAllIngredientsWithQuantity(recipe, userIngredients)
        );
        if (this.DEBUG_MODE)
          console.log(
            `✅ After Strict Available filter: ${filtered.length} recipes`
          );
      }
      ...
      if (mode === "ai_flexible") {
        ...
        filtered = filtered.filter((recipe) =>
          this._hasFlexibleRecommendedIngredients(recipe, userIngredients)
        );
        if (this.DEBUG_MODE)
          console.log(
            `✅ After AI Flexible Recommended filter: ${filtered.length} recipes`
          );
      } else {
        filtered = filtered.filter((recipe) =>
          this._hasRecommendedIngredients(recipe, userIngredients)
        );
        if (this.DEBUG_MODE)
          console.log(
            `✅ After Strict Recommended filter: ${filtered.length} recipes`
          );
      }
    } else if (filter === "All Recipes") {
      if (this.DEBUG_MODE)
        console.log(
          `🎯 Filter is "All Recipes" - keeping all recipes for tag filtering`
        );
    }
    ...
    if (preferredTags.length > 0) {
      ...
      if (this.DEBUG_MODE)
        console.log(
          `🔍 Starting tag filtering with ${
            preferredTags.length
          } preferred tags: [${preferredTags.join(", ")}]`
        );

      // Normalize preferred tags for comparison (trim whitespace and lowercase)
      const normalizedPreferredTags = preferredTags.map((tag) =>
        tag.trim().toLowerCase()
      );
      if (this.DEBUG_MODE)
        console.log(
          `🔧 Normalized preferred tags: [${normalizedPreferredTags.join(
            ", "
          )}]`
        );
      ...
      filtered = filtered.filter((recipe) => {
        ...
        const normalizedRecipeTags = recipeTags.map((tag) =>
          tag.trim().toLowerCase()
        );

        console.log(`🍳 Recipe "${recipe.name}" (ID: ${recipe.id}):`);
        console.log(
          `  📋 Original recipe tags: [${recipeTags.join(", ")}] (${
            recipeTags.length
          } tags)`
        );
        console.log(
          `  🔧 Normalized recipe tags: [${normalizedRecipeTags.join(", ")}]`
        );
        console.log(
          `  🎯 Looking for normalized: [${normalizedPreferredTags.join(", ")}]`
        );
        ...
        const hasMatchingTag = normalizedPreferredTags.every((preferredTag) => {
          ...
          console.log(
            `    🔍 Checking for "${preferredTag}" -> ${matches ? "✅" : "❌"}`
          );
          return matches;
        });

        console.log(
          `  🎯 Final result for "${recipe.name}": ${
            hasMatchingTag ? "✅ INCLUDED" : "❌ EXCLUDED"
          }`
        );
        return hasMatchingTag;
      });
      console.log(
        `🏷️ After tags filter: ${beforeTags} -> ${filtered.length} recipes`
      );

      if (filtered.length === 0) {
        console.log(
          `⚠️ WARNING: No recipes found with normalized tags [${normalizedPreferredTags.join(
            ", "
          )}]`
        );
        ...
        allRecipes.forEach((recipe) => {
          ...
          if (recipeTags.length > 0) {
            console.log(
              `  - "${recipe.name}": original [${recipeTags.join(
                ", "
              )}] -> normalized [${normalizedRecipeTags.join(", ")}]`
            );
          }
        });
      }
    ...
    if (dietaryRestrictions) {
      ...
      filtered = filtered.filter((recipe) =>
        this._meetsDietaryRestrictions(recipe, dietaryRestrictions)
      );
      console.log(
        `🥗 After dietary restrictions filter: ${beforeDietary} -> ${filtered.length} recipes`
      );
    }
    ...
    if (filter !== "With Available Ingredients") {
      if (preferredDifficulty && preferredDifficulty.trim() !== "") {
        ...
        console.log(
          `📊 After difficulty filter: ${beforeDifficulty} -> ${filtered.length} recipes`
        );
      } else {
      ...
      if (
        ...
        filtered = filtered.filter((recipe) => {
          ...
          console.log(
            `⏰ Recipe "${recipe.name}" cooking time: ${recipe.cookingTime} (${cookingTime}min) - Match: ${matches} (limit: ${maxCookingTimeMinutes}min)`
          );
          return matches;
        });
        console.log(
          `⏰ After time filter: ${beforeTime} -> ${filtered.length} recipes`
        );
      } else {
        console.log(
          `⏰ Skipping cooking time filter - no time limit specified (value: ${maxCookingTimeMinutes})`
        );
      }
    } else {
      console.log(
        `🚫 Skipping personal preference filters for "With Available Ingredients" filter`
      );
    }
  ...
  _createUserIngredientLookup(
    userIngredients: UserIngredientOptimizedDto[]
  ): Map<number, UserIngredientOptimizedDto> {
  ...
  _hasAllIngredientsWithQuantity(
    recipe: RecipeDto,
    userIngredients: UserIngredientOptimizedDto[]
  ): boolean {
    if (this.DEBUG_MODE) {
      console.log(
        `🧪 Checking recipe "${recipe.name}" with ${recipe.ingredients.length} ingredients`
      );
      console.log(
        `👤 User has ${userIngredients.length} ingredients available`
      );
    }

    // Create lookup map for O(1) ingredient access instead of O(n) searching
    const userIngredientLookup =
      this._createUserIngredientLookup(userIngredients);

    const result = recipe.ingredients.every((ri, index) => {
      if (this.DEBUG_MODE) {
        console.log(
          `\n  📋 Checking recipe ingredient ${index + 1}/${
      ...
      if (this.DEBUG_MODE) {
        console.log(
          `    🥬 Recipe needs: "${ri.ingredient.name}" (ID: ${ri.ingredient.id})`
        );
        ...
        userIng = userIngredients.find((ui) => {
          ...
          if (ui.customIngredient) {
            ...
            if (recipeName === customName) {
              if (this.DEBUG_MODE)
                console.log(
                  `    🔍 Custom ingredient exact match: "${customName}" vs "${recipeName}" = true`
                );
              return true;
            }
            ...
            const fuzzyMatch =
              recipeName.includes(customName) ||
              customName.includes(recipeName);
            if (this.DEBUG_MODE)
              console.log(
                `    🔍 Custom ingredient fuzzy match: "${customName}" vs "${recipeName}" = ${fuzzyMatch}`
              );
            return fuzzyMatch;
          }
      ...
      if (!userIng.unit || !ri.unit) {
        console.log(
          `    ❌ Missing units - User unit: ${!!userIng.unit}, Recipe unit: ${!!ri.unit}`
        );
        return false;
      }
      ...
      console.log(
        `    🔗 Units compatible: ${compatible} (user: "${userIng.unit.abbreviation}" vs recipe: "${ri.unit.abbreviation}")`
      );
      if (!compatible) {
        return false;
      }
      ...
      try {
        const userAmountBase = this._convertToBase(
          userIng.quantity,
          userIng.unit
        );
        ...
        console.log(
          `    📐 Conversion - User: ${userIng.quantity} ${userIng.unit.abbreviation} = ${userAmountBase} base units`
        );
        console.log(
          `    📐 Conversion - Recipe: ${ri.quantity} ${ri.unit.abbreviation} = ${recipeAmountBase} base units`
        );
    ...
    if (this.DEBUG_MODE)
      console.log(
        `🎯 Recipe "${recipe.name}" result: ${result ? "PASSED" : "FAILED"}\n`
      );
    return result;
  },

  // Función para "Recommended" que verifica ratio y cantidades (como en Flutter)
  _hasRecommendedIngredients(
    recipe: RecipeDto,
    userIngredients: UserIngredientOptimizedDto[]
  ): boolean {
    ...
    if (!userIngredients || userIngredients.length === 0) {
      console.log(
        `  📝 No user ingredients provided - recommending all recipes`
      );
      return true;
    }
    ...
    const validIngredients = recipe.ingredients.filter(
      (ri) => ri.ingredient !== null
    );
    const ratio = matchedCount / validIngredients.length;

    console.log(
      `  📊 Recipe "${recipe.name}": ${matchedCount}/${
        validIngredients.length
      } ingredients matched (${(ratio * 100).toFixed(1)}%)`
    );
    ...
    const hasEnoughMatched = recipe.ingredients.every((ri) => {
      ...
      if (!this._areUnitsCompatible(userIng.unit, ri.unit)) {
        console.log(
          `    🔄 Units not compatible for "${ri.ingredient.name}" - allowing for Recommended filter`
        );
        return true;
      }
      ...
      try {
        ...
        const hasEnough = userAmountBase >= recipeAmountBase;
        console.log(
          `    ✔️ Quantity check for "${ri.ingredient.name}": ${hasEnough}`
        );
        return hasEnough;
      } catch (error) {
        console.log(
          `    🔄 Conversion failed for "${ri.ingredient.name}" - allowing for Recommended filter`
        );
    ...
    console.log(
      `  🎯 Recipe "${
        recipe.name
      }" final result: ${hasEnoughMatched} (ratio: ${(ratio * 100).toFixed(
        1
      )}%)`
    );
  ...
  _hasAlmostAllIngredients(
    ...
    if (this.DEBUG_MODE) {
      console.log(
        `🤖 AI Flexible: Checking recipe "${recipe.name}" (max missing: ${maxMissing})`
      );
    }
    ...
    for (const ri of recipe.ingredients) {
      ...
      if (!userIng) {
        ...
        if (this.DEBUG_MODE) {
          console.log(
            `    ❌ Missing: "${ri.ingredient.name}" (${missingCount}/${maxMissing})`
          );
        }
        ...
        if (missingCount > maxMissing) {
          if (this.DEBUG_MODE) {
            console.log(
              `🤖 Recipe "${recipe.name}" rejected: ${missingCount} missing > ${maxMissing} allowed`
            );
          }
          return false;
        }
    ...
    if (this.DEBUG_MODE) {
      console.log(
        `🤖 Recipe "${recipe.name}" result: ${passed} (${missingCount}/${maxMissing} missing from ${checkedIngredients} ingredients)`
      );
    }
   ...
   */
  _hasFlexibleRecommendedIngredients(
    recipe: RecipeDto,
    userIngredients: UserIngredientOptimizedDto[]
  ): boolean {
    if (!userIngredients || userIngredients.length === 0) {
    ...
    if (this.DEBUG_MODE) {
      console.log(
        `🤖 Flexible Recommended "${
          recipe.name
        }": ${availableCount}/${totalCount} (${(ratio * 100).toFixed(
          1
        )}%, missing: ${missingCount}) → ${passes}`
      );
    }
  ...
  filterRecipesWithMissingData({
    ...
    if (this.DEBUG_MODE) {
      console.log(
        `🤖 AI Detailed filtering: ${filter} (max missing: ${maxMissingIngredients})`
      );
    ...
    if (this.DEBUG_MODE) {
      console.log(
        `🤖 AI Detailed filtering found: ${recipesWithData.length} recipes`
      );
    }
    ...
    if (this.DEBUG_MODE) {
      ...
      filtered.slice(0, 3).forEach((data) => {
        console.log(
          `  📖 "${data.recipe.name}": ${data.availableCount}/${
            data.totalCount
          } ingredients (${data.matchPercentage.toFixed(1)}%, missing: ${
            data.missingCount
          })`
        );
      });
  ...
  _areUnitsCompatible(unit1: any, unit2: any): boolean {
    ...
    if (this.DEBUG_MODE)
      console.log(
        `      🔍 Checking compatibility: "${unit1Abbr}" (${unit1Type}) vs "${unit2Abbr}" (${unit2Type})`
      );
    ...
    if (
      ...
      if (this._areTypesCompatible(unit1Type, unit2Type)) {
        if (this.DEBUG_MODE)
          console.log(
            `      ✅ Compatible by type: ${unit1Type} = ${unit2Type}`
          );
        return true;
      }
    }
    ...
    if (
      containerUnits.includes(unit1Abbr) &&
      containerUnits.includes(unit2Abbr)
    ) {
    ...
    if (
      (weightUnits.includes(unit1Abbr) && measureUnits.includes(unit2Abbr)) ||
      (measureUnits.includes(unit1Abbr) && weightUnits.includes(unit2Abbr))
    ) {
      console.log(
        `      ✅ Weight and measure units are compatible for cooking ingredients`
      );
      return true;
    }
    ...
    if (
      (volumeUnits.includes(unit1Abbr) && measureUnits.includes(unit2Abbr)) ||
      (measureUnits.includes(unit1Abbr) && volumeUnits.includes(unit2Abbr))
    ) {
      console.log(
        `      ✅ Volume and measure units are compatible for cooking ingredients`
      );
      return true;
    }
    ...
    if (this._isCommonSeasoningCompatibility(unit1Abbr, unit2Abbr)) {
      console.log(
        `      ✅ Common seasoning units are compatible for cooking flexibility`
      );
      return true;
    }
  ...
  _areTypesCompatible(type1: string, type2: string): boolean {
    ...
    if (
      (type1Lower === "measure" &&
        (type2Lower === "weight" || type2Lower === "volume")) ||
      (type2Lower === "measure" &&
        (type1Lower === "weight" || type1Lower === "volume"))
    ) {
      return true;
    }
  ...
  _isCommonSeasoningCompatibility(
    unit1Abbr: string,
    unit2Abbr: string
  ): boolean {
    ...
    return (
      (weightUnits.includes(unit1Abbr) &&
        smallVolumeUnits.includes(unit2Abbr)) ||
  ...
  _convertToBase(quantity: number, unit: any): number {
    ...
    console.log(
      `      🔄 Converting ${quantity} ${unitAbbr} (factor: ${factor})`
    );
    ...
    if (unit.type?.toLowerCase() === "measure") {
      console.log(
        `      ⚠️ Using default conversion for measure unit "${unitAbbr}"`
      );
    ...
    if (unitAbbr === "kg" || unitAbbr === "g") {
      console.log(
        `      ⚠️ Converting weight unit "${unitAbbr}" for seasoning compatibility`
      );
    ...
    if (unitAbbr === "tsp" || unitAbbr === "tbsp") {
      console.log(
        `      ⚠️ Converting volume unit "${unitAbbr}" for seasoning compatibility`
      );
  ...
  _meetsDietaryRestrictions(
    ...
    for (const [restriction, required] of Object.entries(restrictions)) {
      ...
      const meetsRestriction = recipe.ingredients.every((ri) => {
        if (!ri.ingredient) {
          console.log(
            `    ⚠️ Ingredient missing for recipe ingredient, assuming safe`
          );
        ...
        console.log(
          `    ${passes ? "✅" : "❌"} "${ingredient.name}" - ${restriction}: ${
            ingredient[restriction as keyof typeof ingredient]
          }`
        );
        return passes;
      });
      ...
      if (!meetsRestriction) {
        console.log(
          `  ❌ Recipe "${recipe.name}" does not meet ${restriction} requirement`
        );
        return false;
      }

      console.log(
        `  ✅ Recipe "${recipe.name}" meets ${restriction} requirement`
      );
    }
